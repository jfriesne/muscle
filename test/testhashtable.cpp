...
int main(int argc, char ** argv)
{
   ...
   if (!isFromScript)
   {
      ...
      {
         Hashtable<uint32, uint32> iTable((PreallocatedItemSlotsCount(NUM_ITEMS)));  // double-parens to avoid most-vexing-parse
   ...
   if (!isFromScript)
   {
      ...
      for (uint32 t=0; t<NUM_RUNS; t++)
      {
         Hashtable<String, String> sTable((PreallocatedItemSlotsCount(NUM_ITEMS)));  // double parens to avoid most-vexing-parse
