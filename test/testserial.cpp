...
int main(int /*argc*/, char ** /*argv*/)
{
   ...
   if (io.IsPortAvailable())
   {
      ...
      while(ret >= 0)
      {
         ret = io.Read(buf, sizeof(buf)).GetByteCount();
         ...
         if (++c == 20)
         {
            ...
            const io_status_t wroteBytes = io.Write(bytes, sizeof(bytes));
            printf("Sent " INT32_FORMAT_SPEC "/%i bytes [%s]\n", wroteBytes.GetByteCount(), (int) sizeof(bytes), wroteBytes.GetStatus()());
            c = 0;
         }
      }
      printf("Done!\n");
