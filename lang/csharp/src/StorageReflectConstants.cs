namespace muscle.client {
  ...
  public abstract class StorageReflectConstants
  {
    ...
    /// Field name (int32 type):  Bit-chord of SETDATANODE_FLAG_* values
    public const string PR_NAME_FLAGS                     = "!SnQ4";

    /// bit-values that can be OR'd in to the PR_NAME_FLAGS field
    public const int SETDATANODE_FLAG_DONTCREATENODE    = 1;  /// SetDataNode() call should error out rather than creating a new DataNode.
    public const int SETDATANODE_FLAG_DONTOVERWRITEDATA = 2;  /// SetDataNode() call should error out rather than overwriting the Message payload of an existing node.
    public const int SETDATANODE_FLAG_QUIET             = 4;  /// SetDataNode() call should suppress the node-updated-notifications that would otherwise be sent out to the node's subscribers
    public const int SETDATANODE_FLAG_ADDTOINDEX        = 8;  /// node should be added to the parent node's ordered-children index.
    public const int SETDATANODE_FLAG_ENABLESUPERCEDE   = 16; /// updates triggered by this action can cancel (and replace) any still-pending earlier updates generated by the same node
