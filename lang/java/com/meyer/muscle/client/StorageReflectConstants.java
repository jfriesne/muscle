...
public interface StorageReflectConstants
{
   ...
   public static final String PR_NAME_FLAGS                     = "!SnQ4";

   /** bit-values that can be OR'd in to the PR_NAME_FLAGS field */
   public static final int SETDATANODE_FLAG_DONTCREATENODE    = (1<<0); // SetDataNode() call should error out rather than creating a new DataNode.
   public static final int SETDATANODE_FLAG_DONTOVERWRITEDATA = (1<<1); // SetDataNode() call should error out rather than overwriting the Message payload of an existing node.
   public static final int SETDATANODE_FLAG_QUIET             = (1<<2); // SetDataNode() call should suppress the node-updated-notifications that would otherwise be sent out to the node's subscribers
   public static final int SETDATANODE_FLAG_ADDTOINDEX        = (1<<3); // node should be added to the parent node's ordered-children index.
   public static final int SETDATANODE_FLAG_ENABLESUPERCEDE   = (1<<4); // updates triggered by this action can cancel (and replace) any still-pending earlier updates generated by the same node
