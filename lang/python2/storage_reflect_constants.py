...
# Field name for archived QueryFilter objects (for applying predicate-logic to subscriptions, etc)
PR_NAME_FILTERS                   = "!SnFl"
...
# Bit-chord of SETDATANODE_FLAG_* values (add the bit-chord value to the Message as an int32)
PR_NAME_FLAGS                     = "!SnQ4"

# bit-values that can be OR'd in to the PR_NAME_FLAGS field
SETDATANODE_FLAG_DONTCREATENODE    = (1<<0) # SetDataNode() call should error out rather than creating a new DataNode.
SETDATANODE_FLAG_DONTOVERWRITEDATA = (1<<1) # SetDataNode() call should error out rather than overwriting the Message payload of an existing node.
SETDATANODE_FLAG_QUIET             = (1<<2) # SetDataNode() call should suppress the node-updated-notifications that would otherwise be sent out to the node's subscribers
SETDATANODE_FLAG_ADDTOINDEX        = (1<<3) # node should be added to the parent node's ordered-children index.
SETDATANODE_FLAG_ENABLESUPERCEDE   = (1<<4) # updates triggered by this action can cancel (and replace) any still-pending earlier updates generated by the same node

# Field name (any type):  If set as parameter, include ourself in wildcard matches
PR_NAME_REFLECT_TO_SELF           = "!Self"

# If set as parameter, session broadcasts unrecognized Messages to neighbors (set by default)
PR_NAME_ROUTE_GATEWAY_TO_NEIGHBORS = "!G2N"

# If set as parameter, session accepts unrecognized Messages from neighbors and sends them to gateway (set by default)
PR_NAME_ROUTE_NEIGHBORS_TO_GATEWAY = "!N2G"
...
# uint64 indicating the server's current wall-clock (microseconds since 1970), in UTC format
PR_NAME_SERVER_CURRENTTIMEUTC     = "!Mct"

# uint64 indicating the server's current wall-clock (microseconds since 1970), in the server's local timezone format
PR_NAME_SERVER_CURRENTTIMELOCAL   = "!Mcl"

# uint64 indicating the server's current run-time clock (microseconds)
PR_NAME_SERVER_RUNTIME            = "!Mrt"

# uint64 that is unique to this particular instance of the server in this particular process
PR_NAME_SERVER_SESSION_ID         = "!Ssi"
...
# Prefix for parameters that indicate a subscription request
PR_NAME_SUBSCRIBE_PREFIX          = "SUBSCRIBE:"

# Identifier field for associating PR_RESULT_DATATREES replies with PR_COMMAND_GETDATATREE commands
PR_NAME_TREE_REQUEST_ID           = "!TRid"

# Parameter name holding int32 of MUSCLE_MESSAGE_ENCODING_* used to send to client
PR_NAME_REPLY_ENCODING            = "!Enc"

# If present as an int32 in PR_COMMAND_GETDATATREES, returned trees will be clipped to this maximum depth. (0==roots only)
PR_NAME_MAXDEPTH                  = "!MDep"
