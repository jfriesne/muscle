...
int main(int argc, char ** argv)
{
   ...
   (void) SetFileLogName(logFileNamesPattern);        // tell the file-logger what kind of files to create
   (void) SetOldLogFilesPattern(logFileNamesPattern); // tell the old-log-files-deleter what kind of old files he is allowed to delete
   (void) SetFileLogMaximumSize(50000);               // don't allow a given log file to be more than 50000 bytes long
   (void) SetMaxNumLogFiles(10);                      // only keep the most recent 30 log files, to avoid using up too much disk space
   //(void) SetFileLogCompressionEnabled(true);       // .gz-compress each log file when closing it, to save disk space
   (void) SetFileLogLevel(MUSCLE_LOG_TRACE);          // log any messages of MUSCLE_LOG_TRACE or higher to disk
   ...
   (void) Snooze64(SecondsToMicros(3));

   uint32 count = 0;
   for (uint32 i=0; i<50; i++)
   {
      (void) Snooze64(MillisToMicros(1100));  // just to give the timestamp time to increase by a second
