...
* When compiled with C++11 or later, [WaitCondition](https://public.msli.com/lcs/muscle/html/classmuscle_1_1WaitCondition.html) is implemented via `std::condition_variable`.  Otherwise, it will be implemented using an appropriate threading API (i.e. `pthreads` or an OS-specific API).
