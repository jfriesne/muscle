...
A [DataUnflattener](https://public.msli.com/lcs/muscle/html/classmuscle_1_1DataUnflattenerHelper.html) is a lightweight binary-data-reader object that makes it easy and safe to read in POD data values from a serialized byte-buffer.

* Instead of copying data-bytes in "by hand" using pointer-arithmetic and memcpy()/ntohl()/etc, you can declare a DataUnflattener on the stack and call ReadInt32()/ReadFloat()/ReadString()/ReadFlat()/etc on it.
...
* Unchecked*DataUnflattener classes are available for those who prefer to do their own manual bounds-checking instead.
