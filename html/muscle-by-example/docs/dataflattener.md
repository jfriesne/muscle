...
A [DataFlattener](https://public.msli.com/lcs/muscle/html/classmuscle_1_1DataFlattenerHelper.html) is a lightweight binary-data-writer object that makes it easy and safe to write POD data values out to a serialized byte-buffer.

* Instead of copying data-bytes out "by hand" using pointer-arithmetic and memcpy()/htonl()/etc, you can declare a DataFlattener on the stack and call WriteInt32()/WriteFloat()/WriteString()/WriteFlat()/etc on it.
...
* Unchecked*DataFlattener classes are available for those who prefer to do their own manual bounds-checking instead.
