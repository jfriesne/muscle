...
public:
   ...
   virtual void InternalThreadEntry()
   {
      ...
      {
         (void) Snooze64(MillisToMicros(GetInsecurePseudoRandomNumber32(10)));  // space things out in time, otherwise we get deadlocks so often that it's hard to run the program to completion
         ...
         else printf("Error, couldn't read/write-lock second ReaderWriterMutex!  (this should never happen!) [%s]\n", r1());
      }
   }
};
