...
#define DECLARE_MUSCLE_CONVENIENCE_METHODS(name, type) \
   ...
   /**
    * Writes the bare payload-data of this Message into a buffer using (flat) and using
    * (templateMsg) as a guide for what field(s) to write.  Note that this method is optimized
    * for minimum data-size, and as such it does NOT write any meta-data at all; rather,
    * the passed-in template-Message represents the meta-data required to flatten or
    * unflatten the generated bytes.
    * @param templateMsg a template/example Message to use to determine what felds get written
   ...
   /** Sets the state of this Message from the flattened templated-payload-data pointed to by
     * (unflat), using the supplied (templateMsg) as a guide for how to interpret the data-bytes.
   ...
   /** Convenience method:  Returns true if the data-bytes pointed to by (bytes) might
