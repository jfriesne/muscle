...
# define DECLARE_MUSCLE_CONVENIENCE_METHODS(name, type) \
    ...
    * @param buffer a pointer to an array of at least TemplatedFlattenedSize(templateMsg)
    *               bytes to write the flattened data into.
    ...
    *       TemplatedUnflatten() so that it can intepret the bytes correctly.
    */
   void TemplatedFlatten(const Message & templateMsg, uint8 * buffer) const;
   ...
   /** Reads the bare payload-data from (buf) into this Message, using the supplied
     ...
     * @param buf the bytes that were previously generated by TemplatedFlatten().
     ...
     */
   status_t TemplatedUnflatten(const Message & templateMsg, const uint8 *buf, uint32 size);
