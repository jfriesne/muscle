...
public:
   ...
   /** Returns true iff (string) is matched by the current expression.
     ...
     * @param prefixMatchOkay if true, Match() will match a pattern that is shorter than the number of
     *                        tokens parsed, as long as the initial tokens match.  For example, a pattern of
     *                        "f??/b??" would match the string "foo/bar/baz", even though there are two tokens
     *                        in the pattern-string and three in the string to be matched.  If false, then
     *                        the number of tokens in the pattern must exactly match the number of tokens in the string.
     * @return true iff (matchString) matches, false otherwise.
     */
   MUSCLE_NODISCARD bool Match(const char * const matchString, bool prefixMatchOkay) const {return _negate ? !MatchAux(matchString, prefixMatchOkay) : MatchAux(matchString, prefixMatchOkay);}
   ...
   /** Convenience method:  Same as above, but takes a String object instead of a (const char *).
     ...
     * @return true iff (matchString) matches, false otherwise.
     */
   MUSCLE_NODISCARD inline bool Match(const String & matchString, bool prefixMatchOkay) const {return Match(matchString(), prefixMatchOkay);}
...
private:
   MUSCLE_NODISCARD bool MatchAux(const char * const matchString, bool prefixMatchOkay) const;
