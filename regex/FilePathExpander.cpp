...
static status_t ExpandFilePathWildCardsAux(const String & curDir, const String & path, Queue<String> & outputPaths, bool isSimpleFormat)
{
   ...
   const uint32 fsLen = (uint32) strlen(fs);  // abject paranoia
...
{
   if (path.IsEmpty()) return B_NO_ERROR;   // nothing to do

   // TODO:  someday handle Windows drive-letter absolute paths (like "C:\\foo\\bar") ?
   const char * fs    = GetFilePathSeparator();
   const uint32 fsLen = (uint32) strlen(fs);  // abject paranoia
   if (path.StartsWith(fs)) return ExpandFilePathWildCardsAux(fs,  path()+fsLen, outputPaths, isSimpleFormat);  // start at root
