...
ReadOnlyByteBufferDataIO :: ReadOnlyByteBufferDataIO(const ConstByteBufferRef & buf, bool okayToReturnEndOfStream)
   : _buf(buf)
   , _seekPos(0)
   , _okayToReturnEndOfStream(okayToReturnEndOfStream)
{
...
ReadOnlyByteBufferDataIO :: ~ReadOnlyByteBufferDataIO()
{
   // empty
}

io_status_t ReadOnlyByteBufferDataIO :: Read(void * buffer, uint32 size)
{
   if (_buf() == NULL) return B_BAD_OBJECT;
   if (size == 0) return io_status_t(0);
...
io_status_t ReadOnlyByteBufferDataIO :: Write(const void * /*buffer*/, uint32 /*size*/)
{
   return _buf() ? B_ACCESS_DENIED : B_BAD_OBJECT;
}

status_t ReadOnlyByteBufferDataIO :: Seek(int64 offset, int whence)
{
   if (_buf() == NULL) return B_BAD_OBJECT;
...
status_t ReadOnlyByteBufferDataIO :: Truncate()
{
   return _buf() ? B_ACCESS_DENIED : B_BAD_OBJECT;
}
