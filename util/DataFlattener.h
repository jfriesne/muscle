...
public:
   ...
   /** Convenience method; writes out all of the bytes inside (buf)
     ...
     */
   void WriteBytes(const ByteBuffer & buf) {WriteBytes(buf.GetBuffer(), buf.GetNumBytes());}
...
///@{
   ...
   void WriteString(const String & val) {WriteStrings(&val, 1);}
///@}
...
///@{
   /** Convenience methods for writing an array of POD-typed data-items to our buffer.
     ...
     */
   void WriteInt8s(const int8 * vals, uint32 numVals) {WriteBytes(reinterpret_cast<const uint8 *>(vals), numVals);}

   void WriteInt16s(const uint16 * vals, uint32 numVals) {WriteInt16s(reinterpret_cast<const int32 *>(vals), numVals);}
   void WriteInt16s(const int16 * vals, uint32 numVals)
   {
   ...
   void WriteInt32s(const uint32 * vals, uint32 numVals) {WriteInt32s(reinterpret_cast<const int32 *>(vals), numVals);}
   void WriteInt32s(const int32 * vals, uint32 numVals)
   {
      for (uint32 i=0; i<numVals; i++)
      {
   ...
   void WriteInt64s(const uint64 * vals, uint32 numVals) {WriteInt64s(reinterpret_cast<const int64 *>(vals), numVals);}
   void WriteInt64s(const int64 * vals, uint32 numVals)
   {
      for (uint32 i=0; i<numVals; i++)
      {
   ...
   void WriteFloats(const float * vals, uint32 numVals)
   {
      for (uint32 i=0; i<numVals; i++)
      {
   ...
   void WriteDoubles(const double * vals, uint32 numVals)
   {
      for (uint32 i=0; i<numVals; i++)
      {
