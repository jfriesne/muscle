...
public:
   ...
   /** Called by GetHostByName() when it wants to resolve a host name (eg "www.google.com" or "blah.local.") into an IPAddress.
     * @param name the hostname string to resolve
     * @param expandLocalhost true iff the caller would prefer that "localhost" should be expanded to a globally meaningful IP address; false if eg 127.0.0.1 or ::1 are preferable.
     ...
     * @return B_NO_ERROR if the IPAddress written to the (retIPAddress) argument should be used, or an error code (eg B_ERROR) if the hostname lookup should be continued by other means if possible.
     * @note that this callback may be called from multiple threads concurrently (eg if multiple threads are calling GetHostByName() concurrently)
     */
