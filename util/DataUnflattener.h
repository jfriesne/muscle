...
private:
   ...
   status_t SizeCheck(uint32 numBytes) {return (numBytes <= GetNumBytesAvailable()) ? B_NO_ERROR : FlagError(B_DATA_NOT_FOUND);}
...
/** This is a version of DataUnflattenerHelper that doesn't do any bounds-checking on the array it reads from.
  * When you use this you should be very careful not to let it read past the end of its array, as doing so
  * will invoke undefined behavior and likely crash the program.
  */
template<class EndianEncoder> class UncheckedDataUnflattenerHelper : public NotCopyable
{
public:
   /** Default constructor.  Create an invalid object.  Call SetBuffer() before using */
   UncheckedDataUnflattenerHelper() {Reset();}

   /** Constructs a UncheckedDataUnflattener that will read up to the specified number of bytes
     * @param readFrom The buffer to read bytes from.  Caller must guarantee that this pointer remains valid when any methods on this class are called.
     * @param maxBytes optional maximum number of bytes that should be read.  Since this class doesn't do bounds checking,
     *                 this value will merely be stored and ignored until our destructor runs -- at that point, if the destructor
     *                 sees that more than (maxBytes) bytes have been read, it will log a critical error message and crash the program.
     *                 This is done to help make bugs more obvious during development.  Defaults to MUSCLE_NO_LIMIT.
     */
   UncheckedDataUnflattenerHelper(const uint8 * readFrom, uint32 maxBytes = MUSCLE_NO_LIMIT) {SetBuffer(readFrom, maxBytes);}
   ...
   /** Same as above, except instead of taking a raw pointer as a target, we take a reference to a ByteBuffer object.
     ...
     *                 sees that more than (maxBytes) bytes have been read, it will log a critical error message and crash the program.
     *                 This is done to help make bugs more obvious during development.  Defaults to MUSCLE_NO_LIMIT.
     * @note if the value of (maxBytes) is greater than (readFrom.GetNumBytes()), it will be treated as equal to (readFrom.GetNumBytes()).
     */
   UncheckedDataUnflattenerHelper(const ByteBuffer & readFrom, uint32 maxBytes = MUSCLE_NO_LIMIT) {SetBuffer(readFrom, maxBytes);}

   /** This destructor will crash the program if it detects that we read past the end of our buffer */
   ~UncheckedDataUnflattenerHelper()
   {
      const uint32 nbr = GetNumBytesRead();
      if (nbr > _maxBytes)
      {
         LogTime(MUSCLE_LOG_CRITICALERROR, "UncheckedDataUnflattenerHelper %p:  " UINT32_FORMAT_SPEC " bytes were read from a buffer that only had space for " UINT32_FORMAT_SPEC " bytes!\n", this, nbr, _maxBytes);
         MCRASH("UncheckedDataUnflattenerHelper detected buffer-read overflow");
      }
   }
   ...
   /** Set a new raw array to read from (same as what we do in the constructor, except this updates an existing UncheckedDataUnflattenerHelper object)
     * @param readFrom the new buffer to point to and read from in future Read*() method-calls.
     * @param maxBytes optional maximum number of bytes that should be read.  Since this class doesn't do bounds checking,
     *                 this value will merely be stored and ignored until our destructor runs -- at that point, if the destructor
     *                 sees that more than (maxBytes) bytes have been read, it will log a critical error message and crash the program.
     ...
     * @note this method resets our status-flag back to B_NO_ERROR.
     */
   void SetBuffer(const uint8 * readFrom, uint32 maxBytes = MUSCLE_NO_LIMIT) {_readFrom = _origReadFrom = readFrom; _maxBytes = maxBytes; _status = B_NO_ERROR;}
   ...
   /** Same as above, except instead of taking a raw pointer as a target, we take a reference to a ByteBuffer object.
     ...
     *                 this value will merely be stored and ignored until our destructor runs -- at that point, if the destructor
     *                 sees that more than (maxBytes) bytes have been read, it will log a critical error message and crash the program.
     *                 This is done to help make bugs more obvious during development.  If this value is greater (readFrom.GetNumBytes())
     *                 it will treated as equal to (readFrom.GetNumBytes()).  Defaults to MUSCLE_NO_LIMIT.
     * @note this method resets our status-flag back to B_NO_ERROR.
     * @note if the value of (maxBytes) is greater than (readFrom.GetNumBytes()), it will be treated as equal to (readFrom.GetNumBytes()).
     */
   void SetBuffer(const ByteBuffer & readFrom, uint32 maxBytes = MUSCLE_NO_LIMIT) {SetBuffer(readFrom.GetBuffer(), muscleMin(readFrom.GetNumBytes(), maxBytes));}
   ...
   /** Returns true iff we have detected any problems reading in data so far (e.g. an object's Unflatten() call failed) */
   status_t GetStatus() const {return _status;}

   /** Reads the specified byte to this UncheckedDataUnflattenerHelper's contents.
     * @param retByte On success, the read byte is written here
     * @returns B_NO_ERROR on success, or B_DATA_NOT_FOUND if no data is available.
     */
   status_t ReadByte(uint8 & retByte) {retByte = *_readFrom; return Advance(sizeof(retByte));}
...
///@{
   ...
   // Gotta implement this separately since ReadFlats() would expect a 4-byte header, which we don't want
   status_t ReadStrings(String * retVals, uint32 numVals)
   {
      for (uint32 i=0; i<numVals; i++)
      {
...
typedef UncheckedDataUnflattenerHelper<LittleEndianEncoder> LittleEndianUncheckedDataUnflattener;  /**< this unchecked unflattener-type unflattens from little-endian-format data */
typedef UncheckedDataUnflattenerHelper<BigEndianEncoder>    BigEndianUncheckedDataUnflattener;     /**< this unchecked unflattener-type unflattens from big-endian-format data */
typedef UncheckedDataUnflattenerHelper<NativeEndianEncoder> NativeEndianUncheckedDataUnflattener;  /**< this unchecked unflattener-type unflattens from native-endian-format data */
