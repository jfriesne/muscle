...
{
   return (GetNumAvailableBytes(currentlyAllocatedBytes) >= allocRequestBytes) ? ProxyMemoryAllocator::AboutToAllocate(currentlyAllocatedBytes, allocRequestBytes) : B_OUT_OF_MEMORY;
}
